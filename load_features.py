...
feature_maps = {
    'iclr18': {
        ...
        'ps_corpus': 'convokit/iclr18_corpus_ps',
    },
    'neurips18': {
        'argument_col': 'NeurIPS_2018',
        'ps_corpus': 'convokit/neurips18_corpus_ps',
    },
    'iclr19': {
        'argument_col': 'ICLR_2019',
        'ps_corpus': 'convokit/iclr19_corpus_ps',
    },
    'neurips19': {
        'argument_col': 'NeurIPS_2019',
        'ps_corpus': 'convokit/neurips19_corpus_ps',
    }
}
...
* Aspect features
* LIWC features
* Convokit features # They do say you would have to do extra work for out of domain
"""


def get_liwc_data(venues):
  # venues = [feature_maps[venue]['argument_col'] for venue in venues]
  file_path = 'liwc_data/liwc_18_19.csv'
...
def get_argument_data(venues):
  venues = [feature_maps[venue]['argument_col'] for venue in venues]
  file_path = 'arguments/predictions.csv'
  df = pd.read_csv(file_path, index_col=0)
  df = df[df['venue'].isin(venues)]
  argument_data = {}
...
def get_all_inverse_specificity_data(venues):
  data = {}
  for venue in venues:
    file_path = 'specificity/{venue}/specificity.json'.format(venue=venue)
    with open(file_path, 'r') as f:
      specificity_list = json.load(f)
      venue_data = {
          s['review_id']: [1 - v for v in s['specificity'] if 0 <= v <= 1
...
def get_mean_specificity_data(venues):
  data = {}
  for venue in venues:
    file_path = 'specificity/{venue}/specificity_mean.csv'.format(venue=venue)
    df = pd.read_csv(file_path, index_col=0)
...
def get_x_review_length(data, feature_type=None):
  """
...
def get_x_decision(data, feature_type=None):
  return 'Accept' in data['decision']


def get_aspect_type(aspect_str, feature_type=None):
  if aspect_str == 'summary':
    return aspect_str
  return '_'.join(aspect_str.split('_')[:-1])


def get_x_aspect_coverage(data, feature_type=None):
  aspect_set = set()
...
def get_x_aspect_recall(data, feature_type=None):
  aspect_set = set()
  meta_aspect_set = set()
...
class Feature(object):
  ...
  ASPECT_FOUND = 'aspect_found'
  ARGUMENT_COUNT = 'argument_count'
  ARGUMENT_FOUND = 'argument_found'


FN_GETTER = {
    Feature.DECISION: get_x_decision,
    Feature.REVIEW_LENGTH: get_x_review_length,
    Feature.ASPECT_COVERAGE: get_x_aspect_coverage,
    Feature.ASPECT_RECALL: get_x_aspect_recall,
    Feature.ASPECT_COUNT: get_x_aspect_count,
    Feature.ASPECT_FOUND: get_x_aspect_found,
    Feature.ARGUMENT_COUNT: get_x_argument_count,
    Feature.ARGUMENT_FOUND: get_x_argument_found,
}
...
def add_feature_column(df, feature_data, feature, feature_type=None):
  ...
  for review_id in df['review_id'].values:
    ...
    val.append(FN_GETTER[feature](data, feature_type))
  df[col] = val
...
def get_labeled_features_df():
  # Load gold_df
  file_path = 'gold_annotations.csv'
  df = pd.read_csv(file_path)
  df['venue'] = df['review_id'].apply(lambda x: 'neurips18'
                                      if 'NIPS' in x else 'iclr18')
  # print(df['annotator_initials'].value_counts())
...
def get_unlabeled_features_df(venue):
  file_path = 'unlabeled/{venue}.csv'.format(venue=venue)
  df = pd.read_csv(file_path, index_col=0)
...
def get_features(df, venues):
  ...
  specificity_data = get_all_inverse_specificity_data(venues)
  ...
  df['word_count'] = df['review_id'].apply(lambda x: liwc_df.loc[x, 'WC'])
  ...
  for col in liwc_df.columns[2:6]:
    df['liwc_' + col] = df['review_id'].apply(lambda x: liwc_df.loc[x, col])
...
def main():
  ...
  if bool(options.labeled):
    ...
    df.to_csv('features/common_ann.csv', index=False)
  else:
    df = get_unlabeled_features_df(options.venue)
    df.to_csv(
        'features/unlabeled_{venue}.csv'.format(venue=options.venue),
        index=False,
    )
